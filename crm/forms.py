...
class checkCustomerForm(ModelForm):
    class Meta:
        model = Customer
        fields = ['username', 'first_name', 'last_name', 'phone', 'email', 'postal_code', 'city', 'address']
...
class CustomerForm(ModelForm):
    ...
    postal_code = RegexField(label = u"کدپستی", help_text = u"کدپستی ده رقمی است", required = True, regex = "\d{10}",
                             error_messages = {'invalid': u"کدپستی شامل 10 رقم است"})
    email = EmailField(label = u"رایانامه", required = True, help_text = "یک رایانامه معتبر مثل: nikbad@sarab.com")
    phone = RegexField(label = u"شماره تماس ثابت",
                       help_text = "تلفن به همراه کد شهرستان 11 رقمی می باشد مثل:0218826299", regex = "\d{7,}",
    ...
    class Meta:
        model = Customer
        fields = ['username', 'password', 'repassword', 'first_name', 'last_name', 'phone', 'email', 'postal_code',
                  'city', 'address']

...
def order_panel(request):
    ...
    context = {'clrs': contacts, 'active_menu': 2, 'first_msg': first_msg, 'paginator': paginator, 'contacts': contacts,
               'lnk': add}
...
def ready_order_panel(request):
    ...
    context = {'clrs': contacts, 'active_menu': 12, 'first_msg': first_msg, 'paginator': paginator,
               'contacts': contacts, 'lnk': add}
    return render(request, 'wrh/Orders-Panel.html', context)
...
def confirm_order(request, pid):
    ...
    if check:
        context = {'error': "این حواله به علت کمبود موجودی انبار آماده تحویل نیست.", 'type': 0,
                   'button': "بازگشت به لیست حواله ها"}
    else:
        clr.ready = 'r'
        clr.save()
        if clr.type == 'warehouse':
...
def delivery_wiki_select_panel(request):
    ...
    context = {'wikis': contacts, 'active_menu': 1, 'paginator': paginator, 'contacts': contacts,
               'lnk': "*WRHDelivery-Panel"}
...
def confirm_wrh_delivery(request):
    ...
    if request.method == 'POST':
        try:
            ...
            if error_num == 2:
                context = {'error': "به علت وارد نشدن نقطه سفارش برای کالای تحویلی، این کالاها در سیستم ثبت نگردیدند.",
                           'button': "ثبت کالاهای تحویلی", 'type': 0}
            elif error_num == 1:
                context = {'error': "به علت وارد نشدن حجم تخمینی برای کالای تحویلی، این کالاها در سیستم ثبت نگردیدند.",
                           'button': "ثبت کالاهای تحویلی", 'type': 0}
            else:
                count = 0
                for dic_key in qnts.keys():
                    if qnts[dic_key] > 0:
                        ...
                        if vl:
                            if not stck:
                                st = Stock(product = prd, quantity = vl, quantity_returned = 0, rack_num_returned = 0,
                                           rack_num = 0)
                                st.save()
                            else:
                ...
                if count == 0:
                    context = {'error': "به علت صفر بودن تعداد کالاهای وارد شده، ورودی به انبار ثبت نگردید.",
                               'button': "ثبت کالاهای تحویلی", 'type': 0}
                else:
                    context = {'msg': "کالاهای تحویلی مربوط به ویکی انتخاب شده با موفقیت در سیستم ثبت شدند.",
                               'button': "ثبت کالاهای جدید"}
                    # else:
                        ...
                        if clr.type == 'sale':
                            ...
                            if clr.type == 'warehouse':
                                if clr.transfer.bill.deliveryStatus == 2:
                                    ...
                                    tr = Transference.objects.filter(bill = (clr.transfer.bill), product = pr,
                                                                     defective = 'd')
                                    if tr.all():
                                        context = {
                                        'error': "تعداد کالای مرجوعی از این نوع برای حواله انتخاب شده قبلا در سیستم به ثبت رسیده است."}
...
def confirm_return(request, pid, kid, cid, type):
    ...
    if qnt == 0:
        context = {'error': "لطفا عددی بزرگتر از صفر برای مقدار کالای معیوب وارد کنید.", 'pid': pid, 'kid': kid,
                   'type': 2}
    elif quan < qnt:
        context = {'error': "مقدار وارد شده برای کالای مرجوعی بیش تر از مقدار ثبت شده در حواله است.", 'pid': pid,
                   'kid': kid, 'type': 2}
    else:
        try:
            ...
            if tp:
                ...
                if clr.type == 'warehouse':
                    ...
                    if clr.type == 'warehouse':
                        ...
                        tr.save()
                context = {'msg': "کالاهای مرجوعی به موجودی انبار اضافه شده و تحویل آن ها در سیستم ثبت گردید.",
                           'button': "ثبت مرجوعی جدید"}
            stck.save()
        except Exception as e:
            print(str(e))
...
def printt(request):
    ...
    for pr in prs:
        ...
        w += 1
        if w == 1:
            tt = "123" + str((datetime.datetime.now().year)) + str((datetime.datetime.now().month)) + str(
                (datetime.datetime.now().day)) + str(datetime.datetime.time(datetime.datetime.now()).hour) + str(
                datetime.datetime.time(datetime.datetime.now()).minute)
            a[j].append(StringMark(224, -158, tt))
            a[j].append(StringMark(485, -128, jalali(datetime.datetime.now().date())))
            a[j].append(StringMark(360, -128, str(datetime.datetime.time(datetime.datetime.now()).hour) + ":" + str(
                datetime.datetime.time(datetime.datetime.now()).minute)))
            a[j].append(StringMark(184, -128, get_user(request).first_name))
            www = u'صفحه '
            www += str(w)
        ...
        if w == 26:
            ...
            a.append([])
            i = 0
    return getPDF_Response(a, os.path.join(settings.MEDIA_ROOT, 'PDFs/anbaargardani.pdf'), pageSize = letter,
...
def print_dif(request):
    ...
    for pr, pk, cnt, cal, dif, type in anbaargardaani_dif:
        ...
        w += 1
        if w == 1:
            tt = "234" + str((datetime.datetime.now().year)) + str((datetime.datetime.now().month)) + str(
                (datetime.datetime.now().day)) + str(datetime.datetime.time(datetime.datetime.now()).hour) + str(
                datetime.datetime.time(datetime.datetime.now()).minute)
            a[j].append(StringMark(224, -158, tt))
            a[j].append(StringMark(485, -128, jalali(datetime.datetime.now().date())))
        ...
        if w == 26:
            ...
            a.append([])
            i = 0
    return getPDF_Response(a, os.path.join(settings.MEDIA_ROOT, 'PDFs/anbaargardaani-dif.pdf'), pageSize = letter,
                           orientation = landscape)
...
def print_customer_receipt(request, clr_id):
    ...
    a[0].append(StringMark(360, -133, str(datetime.datetime.time(datetime.datetime.now()).hour) + ":" + str(
        datetime.datetime.time(datetime.datetime.now()).minute)))
    a[0].append(StringMark(184, -134, get_user(request).first_name))
    www = u'صفحه 1'
    ...
    return getPDF_Response(a, os.path.join(settings.MEDIA_ROOT, 'PDFs/reCdeMoshtari.pdf'), pageSize = letter,
                           orientation = landscape)
...
def print_wiki_receipt(request, clr_id):
    ...
    return getPDF_Response(a, os.path.join(settings.MEDIA_ROOT, 'PDFs/reCdeWiki.pdf'), pageSize = letter,
                           orientation = landscape)
...
def print_stock(request):
    ...
    for s in st:
        ...
        w += 1
        if w == 1:
            tt = "345" + str((datetime.datetime.now().year)) + str((datetime.datetime.now().month)) + str(
                (datetime.datetime.now().day)) + str(datetime.datetime.time(datetime.datetime.now()).hour) + str(
                datetime.datetime.time(datetime.datetime.now()).minute)
            a[j].append(StringMark(224, -158, tt))
            a[j].append(StringMark(485, -127, jalali(datetime.datetime.now().date())))
            a[j].append(StringMark(360, -127, str(datetime.datetime.time(datetime.datetime.now()).hour) + ":" + str(
                datetime.datetime.time(datetime.datetime.now()).minute)))
            a[j].append(StringMark(184, -127, get_user(request).first_name))
            www = u'صفحه '
            www += str(w)
            a[j].append(StringMark(320, 570, www))
        a[j].append(StringMark(487 + len(str(index)) * 3, -51 + i, index))
        ...
        a[j].append(StringMark(240 + len(u''.join(s.product.wiki.companyName)) * 3, -51 + i,
                               u''.join(s.product.wiki.companyName)))
        ...
        if w == 26:
            ...
            a.append([])
            i = 0
    return getPDF_Response(a, os.path.join(settings.MEDIA_ROOT, 'PDFs/report-stock.pdf'), pageSize = letter,
                           orientation = landscape)
...
def print_damaged(request):
    ...
    for s in st:
        ...
        w += 1
        if w == 1:
            tt = "456" + str((datetime.datetime.now().year)) + str((datetime.datetime.now().month)) + str(
                (datetime.datetime.now().day)) + str(datetime.datetime.time(datetime.datetime.now()).hour) + str(
                datetime.datetime.time(datetime.datetime.now()).minute)
            a[j].append(StringMark(224, -158, tt))
            a[j].append(StringMark(485, -127, jalali(datetime.datetime.now().date())))
        ...
        if w == 26:
            ...
            a.append([])
            i = 0
    return getPDF_Response(a, os.path.join(settings.MEDIA_ROOT, 'PDFs/report-damaged.pdf'), pageSize = letter,
                           orientation = landscape)
...
def print_wiki_order(request):
    ...
    for s in st:
        ...
        w += 1
        if w == 1:
            tt = "567" + str((datetime.datetime.now().year)) + str((datetime.datetime.now().month)) + str(
                (datetime.datetime.now().day)) + str(datetime.datetime.time(datetime.datetime.now()).hour) + str(
                datetime.datetime.time(datetime.datetime.now()).minute)
            a[j].append(StringMark(224, -158, tt))
            a[j].append(StringMark(485, -133, jalali(datetime.datetime.now().date())))
            a[j].append(StringMark(360, -133, str(datetime.datetime.time(datetime.datetime.now()).hour) + ":" + str(
                datetime.datetime.time(datetime.datetime.now()).minute)))
            a[j].append(StringMark(184, -134, get_user(request).first_name))
            www = u'صفحه '
            www += str(w)
            a[j].append(StringMark(320, 570, www))
        ...
        a[j].append(
            StringMark(287 + len(u''.join(s.wiki.companyName)) * 3, -51 + i, u''.join(s.product.wiki.companyName)))
        ...
        if w == 26:
            ...
            a.append([])
            i = 0
    return getPDF_Response(a, os.path.join(settings.MEDIA_ROOT, 'PDFs/report-wiki-order.pdf'), pageSize = letter,
                           orientation = landscape)
...
def print_wrh_delivery(request):
    ...
    for s in st:
        ...
        w += 1
        if w == 1:
            tt = "678" + str((datetime.datetime.now().year)) + str((datetime.datetime.now().month)) + str(
                (datetime.datetime.now().day)) + str(datetime.datetime.time(datetime.datetime.now()).hour) + str(
                datetime.datetime.time(datetime.datetime.now()).minute)
            a[j].append(StringMark(224, -158, tt))
            a[j].append(StringMark(485, -127, jalali(datetime.datetime.now().date())))
            a[j].append(StringMark(360, -127, str(datetime.datetime.time(datetime.datetime.now()).hour) + ":" + str(
                datetime.datetime.time(datetime.datetime.now()).minute)))
            a[j].append(StringMark(184, -126, get_user(request).first_name))
            www = u'صفحه '
            www += str(w)
            a[j].append(StringMark(320, 570, www))
        ...
        a[j].append(
            StringMark(273 + len(u''.join(s.wiki.companyName)) * 3, -51 + i, u''.join(s.product.wiki.companyName)))
        ...
        if w == 26:
            ...
            a.append([])
            i = 0
    return getPDF_Response(a, os.path.join(settings.MEDIA_ROOT, 'PDFs/report-wrh-delivery.pdf'), pageSize = letter,
                           orientation = landscape)
...
def print_wrh_clear(request):
    ...
    for s in st:
        ...
        w += 1
        if w == 1:
            tt = "789" + str((datetime.datetime.now().year)) + str((datetime.datetime.now().month)) + str(
                (datetime.datetime.now().day)) + str(datetime.datetime.time(datetime.datetime.now()).hour) + str(
                datetime.datetime.time(datetime.datetime.now()).minute)
            a[j].append(StringMark(224, -158, tt))
            a[j].append(StringMark(485, -127, jalali(datetime.datetime.now().date())))
        ...
        if s.clearance.type == 'sale':
            ...
            a[j].append(
                StringMark(340 + len(str(s.clearance.bill.customer.pk)) * 2.5, -51 + i, s.clearance.bill.customer.pk))
            ...
            a[j].append(StringMark(340 + len(str(s.clearance.transfer.bill.customer.pk)) * 2.5, -51 + i,
                                   s.clearance.transfer.bill.customer.pk))
        else:
        ...
        if w == 26:
            ...
            a.append([])
            i = 0
    return getPDF_Response(a, os.path.join(settings.MEDIA_ROOT, 'PDFs/report-wrh-clearance.pdf'), pageSize = letter,
                           orientation = landscape)
...
def print_custrcpt(request):
    ...
    for s in st:
        ...
        w += 1
        if w == 1:
            tt = "890" + str((datetime.datetime.now().year)) + str((datetime.datetime.now().month)) + str(
                (datetime.datetime.now().day)) + str(datetime.datetime.time(datetime.datetime.now()).hour) + str(
                datetime.datetime.time(datetime.datetime.now()).minute)
            a[j].append(StringMark(224, -158, tt))
            a[j].append(StringMark(490, -125, jalali(datetime.datetime.now().date())))
            a[j].append(StringMark(365, -125, str(datetime.datetime.time(datetime.datetime.now()).hour) + ":" + str(
                datetime.datetime.time(datetime.datetime.now()).minute)))
            a[j].append(StringMark(189, -126, get_user(request).first_name))
            www = u'صفحه '
            www += str(w)
            a[j].append(StringMark(320, 570, www))
        ...
        if s.clearance.type == 'sale':
            ...
            name += u''.join(s.clearance.bill.customer.last_name)
            a[j].append(
                StringMark(332 + len(str(s.clearance.bill.customer.pk)) * 2.5, -51 + i, s.clearance.bill.customer.pk))
        elif s.clearance.type == 'warehouse':
            name = u''.join(s.clearance.transfer.bill.customer.first_name)
            name += " "
            name += u''.join(s.clearance.transfer.bill.customer.last_name)
            a[j].append(StringMark(332 + len(str(s.clearance.transfer.bill.customer.pk)) * 2.5, -51 + i,
                                   s.clearance.transfer.bill.customer.pk))
        else:
            name = u''.join(s.clearance.wiki.wiki.companyName)
        ...
        if w == 26:
            ...
            a.append([])
            i = 0
    return getPDF_Response(a, os.path.join(settings.MEDIA_ROOT, 'PDFs/report-receipt-customer.pdf'), pageSize = letter,
                           orientation = landscape)
...
def print_WikiRcp(request):
    ...
    for s in st:
        ...
        w += 1
        if w == 1:
            tt = "901" + str((datetime.datetime.now().year)) + str((datetime.datetime.now().month)) + str(
                (datetime.datetime.now().day)) + str(datetime.datetime.time(datetime.datetime.now()).hour) + str(
                datetime.datetime.time(datetime.datetime.now()).minute)
            a[j].append(StringMark(224, -158, tt))
            a[j].append(StringMark(485, -127, jalali(datetime.datetime.now().date())))
        ...
        if s.clearance.type == 'sale':
            ...
            name += u''.join(s.clearance.bill.customer.last_name)
            a[j].append(
                StringMark(321 + len(str(s.clearance.bill.customer.pk)) * 2.5, -51 + i, s.clearance.bill.customer.pk))
        elif s.clearance.type == 'warehouse':
            name = u''.join(s.clearance.transfer.bill.customer.first_name)
            name += " "
            name += u''.join(s.clearance.transfer.bill.customer.last_name)
            a[j].append(StringMark(321 + len(str(s.clearance.transfer.bill.customer.pk)) * 2.5, -51 + i,
                                   s.clearance.transfer.bill.customer.pk))
        else:
            name = u''.join(s.clearance.wiki.wiki.companyName)
        ...
        if w == 26:
            ...
            a.append([])
            i = 0
    return getPDF_Response(a, os.path.join(settings.MEDIA_ROOT, 'PDFs/report-receipt-wiki.pdf'), pageSize = letter,
                           orientation = landscape)
...
def print_Hvl(request):
    ...
    for s in st:
        ...
        w += 1
        if w == 1:
            tt = "102" + str((datetime.datetime.now().year)) + str((datetime.datetime.now().month)) + str(
                (datetime.datetime.now().day)) + str(datetime.datetime.time(datetime.datetime.now()).hour) + str(
                datetime.datetime.time(datetime.datetime.now()).minute)
            a[j].append(StringMark(224, -158, tt))
            a[j].append(StringMark(485, -127, jalali(datetime.datetime.now().date())))
        ...
        if s.type == 'sale':
            ...
            a[j].append(
                StringMark(375 + len(str(s.transfer.bill.customer.pk)) * 2.5, -51 + i, s.transfer.bill.customer.pk))
        else:
        ...
        if w == 26:
            ...
            a.append([])
            i = 0
    return getPDF_Response(a, os.path.join(settings.MEDIA_ROOT, 'PDFs/report-receipt-havale.pdf'), pageSize = letter,
                           orientation = landscape)
            ...
            for dic_key in qnts_dmgd:
                ...
                if not stc:
                    ...
                    if stc[0].quantity_returned != qnts_dmgd[dic_key]:
                        tmp.append((pr, pr.pk, qnts_dmgd[dic_key], stc[0].quantity_returned,
                                    qnts_dmgd[dic_key] - stc[0].quantity_returned, 1))
        except Exception as s:
            print(str(s))
...
def report_receipt_panel(request, menu_id, num = 0):
    ...
    if tmp == 88:
        ...
        st = Receipt_Customer_Wiki.objects.filter(clearance__in = Clearance.objects.filter(type = 'sale')).order_by(
            '-date')
        ...
        st = Receipt_Customer_Wiki.objects.filter(clearance__in = Clearance.objects.filter(type = 'wiki')).order_by(
            '-date')
    ...
    # add += str(tmp)
    context = {'dls': contacts, 'active_menu': tmp, 'first_msg': first_msg, 'paginator': paginator,
               'contacts': contacts, 'lnk': add, 'print': prt}
...
def report_product_panel(request, menu_id, num = 0):
    ...
    context = {'stocks': contacts, 'active_menu': tmp, 'first_msg': first_msg, 'paginator': paginator,
               'contacts': contacts, 'print': prt}
...
def report_receipt_delivery2(request, org = "", num = 0):
    ...
    context = {'dls': contacts, 'active_menu': 8, 'first_msg': first_msg, 'lnk': add, 'paginator': paginator,
               'contacts': contacts}
    return render(request, 'wrh/ReportReceipt-Panel.html', context)
...
def report_receipt_delivery_not2(request, org = "", num = 0):
    ...
    context = {'dls': contacts, 'active_menu': 15, 'first_msg': first_msg, 'lnk': add, 'paginator': paginator,
               'contacts': contacts}
    return render(request, 'wrh/ReportReceipt-Panel.html', context)

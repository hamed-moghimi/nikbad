...
def is_harf(char):
    return ord(char) in range(0x0600, 0x06FF + 1) or ord(char) in range(0xFE70, 0xFEFF + 1) or \
           char in "گچپژ"

#numbers from unicode charts (links below)
...
def get_contextual_shape(prev, harf, next):
    ...
    if next_connection and prev_connection:
        return harf.medial
    elif next_connection and not prev_connection:
        return harf.initial
    elif prev_connection and not next_connection:
        return harf.final
    else:
        return harf.isolated
...
forms4(u'\u067E', u'\uFB56') # pe
forms4(u'\u0686', u'\uFB7A') # che
forms4(u'\u06CC', u'\uFBFC') # ye farsi
forms4(u'\u06AF', u'\uFB92') # ge
forms4(u'\u06A9', u'\uFB8E') # ke farsi

# end Darioush edition
